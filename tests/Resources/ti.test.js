...
describe('Titanium', function () {
	...
	it('#addEventListener() for multiple callbacks', function (finish) {
		var count = 0;
		var func = function() {
			count++;
			if (count == 2) {
		        Ti.removeEventListener('ti_event_test', func); // This should remove all callbacks
		        Ti.fireEvent('ti_event_test', {}); // This should not invoke further events
				finish();
			} else if (count >= 3) {
				finish(new Error('removeEventListener did not remove callback'));
			}
		};
		Ti.addEventListener('ti_event_test', func);
		Ti.addEventListener('ti_event_test', func);

		Ti.fireEvent('ti_event_test', {});
	});
