...
-%>
			...
			<%- include('native_class_to_js.cpp', {type: type, metadata: metadata, to_assign: to_assign, argument_name: to_assign + '_iter' }) -%>
<%
...
-%>
			auto <%= to_assign %> = context.CreateObject(JSExport<<%- type.replace(/\./g, '::') %>>::Class());
