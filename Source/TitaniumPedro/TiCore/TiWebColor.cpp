...
// Code inspired by our iOS SDK
Windows::UI::Color Ti::WebColor::ColorForHex(const std::string& hexCode) {
	unsigned length = hexCode.size();
	char alpha = 255;
	if ((length != 3) && (length != 4) && (length != 6) && (length != 7) && (length != 8)) {
		// TODO: make this nicer
		std::wstring message(L"[WARN] Hex color passed looks invalid: ");
		message += std::wstring(hexCode.begin(), hexCode.end());
...
Windows::UI::Color Ti::WebColor::Color(const std::string& colorName) {
	// pre condition
	...
	if (colorName[0] == '#') {
		return ColorForHex(colorName.substr(1));
	}
