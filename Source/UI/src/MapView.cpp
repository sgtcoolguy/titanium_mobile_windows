...
namespace TitaniumWindows
{
	namespace UI
	{
			...
			mapview__(ref new MapControl())
		{
			...
			if (arguments.size() >= 1 && arguments[0].IsObject()) {
				...
				if (arg.HasProperty("mapType")) {
					...
					mapType__ = static_cast<int>(prop);
				}
		...
		void MapView::setMapType(int mapType) TITANIUM_NOEXCEPT {
			...
			if (mapType__ == static_cast<int>(Titanium::MapModule::MapType::HYBRID)) {
				mapview__->Style = MapStyle::AerialWithRoads;
			} else if (mapType__ == static_cast<int>(Titanium::MapModule::MapType::SATELITE)) {
				mapview__->Style = MapStyle::Aerial;
			} else if (mapType__ == static_cast<int>(Titanium::MapModule::MapType::STANDARD)) {
		...
		void MapView::setZoomLevel(int zoom) TITANIUM_NOEXCEPT {
			Titanium::UI::MapView::setZoomLevel(zoom);
