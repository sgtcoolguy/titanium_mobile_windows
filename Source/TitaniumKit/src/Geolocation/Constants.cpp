...
namespace Titanium
{
	namespace Geolocation
	{
		...
		AUTHORIZATION Constants::to_AUTHORIZATION(const std::string& fieldTypeName) TITANIUM_NOEXCEPT
		{
			...
			static std::once_flag of;
			std::call_once(of, []() {
				map[""] = AUTHORIZATION::ALWAYS;
				map[""] = AUTHORIZATION::AUTHORIZED;
				map[""] = AUTHORIZATION::DENIED;
				map[""] = AUTHORIZATION::RESTRICTED;
				map[""] = AUTHORIZATION::UNKNOWN;
				map[""] = AUTHORIZATION::WHEN_IN_USE;
			});
