...
theMovieDb.common = {
    api_key: "b31a733f2ca3cca5cf04cfdf1650a6d0",
    ...
    validateRequired: function(args, argsReq, opt, optReq, allOpt) {
        ...
        if (argsReq > 2) for (i = 0; i < optReq.length; i += 1) if (!opt.hasOwnProperty(optReq[i])) throw optReq[i] + " is a required parameter and is not present in the options!";
    },
    ...
    client: function(options, success, error) {
        ...
        xhr.onload = function() {
            4 === xhr.readyState && xhr.status === status ? success(xhr.responseText) : error(xhr.responseText);
        };
        xhr.onerror = function() {
            error(xhr.responseText);
        };
        xhr.send("POST" === options.method ? JSON.stringify(options.body) : null);
    }
};
