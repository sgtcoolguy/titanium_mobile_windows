...
function exportYAML() {
	for (module_name in titaniumKit) {
		...
		lines.push('platforms: [windowsstore, windowsphone]');

		// dump properties
		titaniumKit[module_name].properties && lines.push('properties:');
		...
		// dump methods
		titaniumKit[module_name].methods && lines.push('methods:');
		...
		// dump events
		titaniumKit[module_name].events && lines.push('events:');
...
extract(function() {
	...
	for (name in titaniumKit) {
		var prop = windowsTitaniumKit[name];
		if (prop) {
			if (prop.properties.length == 0 && prop.methods.length == 0 && prop.events.length == 0) {
				delete titaniumKit[name];
			} else {
				titaniumKit[name] = prop;
			}
		}
	}
